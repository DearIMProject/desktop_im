...
class ByteBuf {
  ...
  ByteBuf(int initSize) {
    if (initSize < initMinSize) {
      initSize = initMinSize;
    }

    _data = Uint8List(initSize);
  }
  ...
  int readInt8() {
    ...
    Log.error("Error! read out of range");
    return 0;
  }
  ...
  String? readString() {
    ...
    return Uint8ListUtils.convertUint8ListToString(strBytes);
  }

  //读取指定数量的byte为Uint8List
  Uint8List readUint8List(int readSize) {
    if (readSize < 1) {
      Log.error("Error Read Size invalidate.");
      return Uint8List(1);
    }
    ...
    Log.error("Error readUint8List! read out of range.");
    return Uint8List(1);
  }
  ...
  void debugPrint() {
    ...
    Log.debug("$str  r = $_readIndex ,w = $_writeIndex");
  }
  ...
  void debugHexPrint({int columSize = 32}) {
    ...
    for (int i = 0; i < _writeIndex; i++) {
      ...
      sb += ("${Uint8ListUtils.intToHex(_data[i])} ");
    }
    //stdout.write("\n" + ("="*columSize));
    sb += ("\n${"=" * columSize}");
    Log.debug(sb);
